...
❯ go 中 chan 收信息的 ok 是什么作用？v, ok := <-c3

v, ok := <-c3 里的 ok 用来区分通道是否已经关闭。

含义

- ok == true：正常从通道收到了一个值(是别的 goroutine 发送进来的)。
- ok == false:通道已经被关闭,并且缓冲区里的值也已经取完了。此时 v 是该类型的零值(比如 int 得到 0,指针得到 nil)。

为什么需要它

只写 v := <-c3 时,如果通道关闭,你会一直收到零值,却无法判断这个零值是「真  」。ok 就是用来消除这个歧义的。

c3 := make(chan int, 2)
c3 <- 10
close(c3)

v, ok := <-c3 // v=10, ok=true  (关闭前发的值还能取出来)
v, ok = <-c3  // v=0,  ok=false (缓冲区空了且已关闭)
v, ok = <-c3  // v=0,  ok=false (之后一直如此,且不会阻塞)

注意:从已关闭的通道接收不会阻塞、不会 panic;只有向已关闭通道发送才会 pani

常见用法:配合 for-range 或手动退出
...
// for-range 等价写法,通道关闭时自动退出,拿不到 ok
for v := range c3 {
fmt.Println(v)
}

for range 通道时就是在内部用这个 ok 机制来决定何时结束的。